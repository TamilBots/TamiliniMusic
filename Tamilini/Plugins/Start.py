...
from Tamilini import ASSID, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Tamilini import boottime as bot_start_time
from Tamilini import db
from Tamilini.Core.PyTgCalls import Tamilini
from Tamilini.Database import (add_nonadmin_chat, add_served_chat,
...
from Tamilini.Decorators.admins import ActualAdminCB
from Tamilini.Decorators.permission import PermissionCheck
from Tamilini.Inline import (custommarkup, dashmarkup, setting_markup,
                          start_pannel, usermarkup, volmarkup)
from Tamilini.Utilities.ping import get_readable_time
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await Tamilini.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Tamilini Disk Usage: {diske}%", show_alert=True
        )
