...
from Tamilini import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from Tamilini.Core.PyTgCalls import Queues, Tamilini
from Tamilini.Core.PyTgCalls.Converter import convert
from Tamilini.Core.PyTgCalls.Downloader import download
from Tamilini.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from Tamilini.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Tamilini.Decorators.admins import AdminRightsCheckCB
from Tamilini.Decorators.checker import checkerCB
from Tamilini.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from Tamilini.Utilities.changers import time_to_seconds
from Tamilini.Utilities.chat import specialfont_to_normal
from Tamilini.Utilities.paste import isPreviewUp, paste_queue
from Tamilini.Utilities.theme import check_theme
from Tamilini.Utilities.thumbnails import gen_thumb
from Tamilini.Utilities.timer import start_timer
from Tamilini.Utilities.youtube import get_yt_info_id
    ...
    if not _playlist:
        ...
        for TamilBots in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await Tamilini.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for TamilBots in _playlist:
            await delete_playlist(user_id, TamilBots, genre)
    await CallbackQuery.message.reply_text(
